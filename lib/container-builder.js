...
var ContainerBuilder = function(serviceLoader){
  ...
  loadServiceFromDefinition = function(definition){
    ...
    if(definition.hasCalls()){
      ...
      for(var i=0, j=calls.length; i<j; i++){
        ...
        if (!(method in obj) || typeof obj[method] !== 'function') {
          throw new Error(
              'Invalid service definition call for ' + definition.getId() +': ' + method + ' is not a function.');
        }
    ...
    if (definition.hasAlias()){
      __container.set(definition.getAlias(), obj);
      __loadedServices[definition.getAlias()] = obj;
    }
